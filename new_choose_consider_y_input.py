...
# while(v>23):
    # print("gogowhile")
    # print(use_year_input)
# print(toolowreward)
    toolowreward = expect_reward - 0.025
    if toolowreward >0.02:
        toolowreward = 0.02

    for i in range(len(v1)):
        if(reward[i]<toolowreward):
            # print(reward[i])
            record_v1[i] = 1
    ...
    # print(record_v1)
    # print(count_no_record)
    
    if count_no_record == 0:
        break

    count_risk_high = 0
    if count_no_record>20:
        if count_no_record>22:
            count_risk_high = -3
        while(count_risk_high<4 and count_no_record>16):
            risktoohigh = std_find_too_high.index(max(std_find_too_high))
            record_v1[risktoohigh] = 1
            std_find_too_high[risktoohigh] = -1
            count_no_record-=1
            count_risk_high+=1

    for i in range(len(v1)):
        std_find_too_high[i] = std[i]
    ...
    for j in range(len(final_name1)):
        final_reward += reward_nodiv[v1.index(final_name[j])]*final_w[j]
        final_div += div[v1.index(final_name1[j])]*final_w[j]
    ...
    if float(min_risk)==100 and use_year_input==1:
        result1 = ""
        # print("no result")
        break
    elif float(min_risk)==100 and use_year_input==0:
        # print("run year input")
        use_year_input=1
        expect_reward = expect_reward_y
        v=24
        for z in range(len(reward)):
            reward[z]=reward_yin[z]
